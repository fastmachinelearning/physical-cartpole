...
def run_step3_launcher(repo: Path, net_name: str | None) -> None:
    script = repo / "scripts" / "demos" / "step3.sh"
...
def set_automation_scripts_executable(repo: Path) -> None:
    for path in [
        repo / "scripts" / "install_zybo_board.sh",
        repo / "tcl" / "generate_bitstream.tcl",
        repo / "tcl" / "generate_vitis_project.tcl",
    ]:
...
def _to_c_float_literal(v: float) -> str:
    lit = f"{float(v):.9g}"
...
def apply_step63_normalization_patch(
    ...
    if apply_normalization_patch:
        ...
        for key, fp in files.items():
            ...
            body = ", ".join(_to_c_float_literal(v) for v in vals)
...
def run_step64_bitstream(
    ...
    bitstream_tcl = resolve_tcl_for_current_repo(
        repo=repo,
        src_tcl=repo / "tcl" / "generate_bitstream.tcl",
        impl_tmp=impl_tmp,
        hls_output_abs=hls_output_abs,
    )
    ...
    _run_cmd_live(["bash", repo / "scripts" / "install_zybo_board.sh"], cwd=repo)
